...
try:
    ...
    import pickle
try:
    sysos.path.append('/home/lv70806/deniz/upload/pypresso_testing/raspberry_filaments')
...
parser.add_argument('-alpha', '--alpha', type=float, required=True,
                    help='float equilibrium length of FENE')
parser.add_argument('-len', '--len', type=int, required=True,
                    help='int N of particles')
parser.add_argument('-h_field', '--h_field', type=float, required=True,
                    help='float ext H field strength in along z-axis')
parser.add_argument('-eta', '--eta', type=float, required=True, help='kinematic viscosity')
parser.add_argument('-init', '--init', nargs=3, type=int, required=True, help='channel pos')
parser.add_argument('-slip_v', '--slip_v',nargs=3, type=float, required=True, help='slip velocitity of z wall')
...
def init_filament_params(filament):
    ...
    param_dict = dict(num_particles=None, config=[], init_pos=(None, None, None),
                      type_nonmag=None, type_mag=None, type_virt=None, type_to_be_magnetized=None, fene_k=None,
                      fene_r_max=None, fene_r0=None, bond_len=None, ang_seeds=(None, None), dip_moments=(None, None, None),
                      dip_magnitude=None, magnetize=None, sinTheta=None, cosTheta=None, cosPhi=None,
                      sinPhi=None, last_index_used=None, realz_indices=[])
...
def set_filament_params(filament):
    ...
    # filament.__dict__['bond_len'] = core.sigma*pow(2, 1/6.)
    filament.__dict__['fene_k'] = 142
...
def gen_positions():
    ...
    with open(os.path.join(sysos.path[-1], 'dungeon_witch_raspberry.txt'), mode='r') as source:
        summary = source.readlines()
        for line in summary:
            bla, iks, ipsilon, zed = line.strip().split(sep=' ')
...
def init_sys_params(core):
    ...
    param_dict = dict(periodicity=(None, None, None), time_step=None, box_l=(None, None, None), skin=None,
                      sigma=None, min_global_cut=None, temperature=None, gamma=None, seed=None,
...
def set_sys_params(core):
    ...
    core.__dict__['sigma'] = 3.0
    core.__dict__['min_global_cut'] = 1.
...
class Filament():
            ...
            for (index, element) in enumerate(grid_of_indices):
                if index in self.attributes.realz_indices:
                    if self.attributes.magnetize == True:
                        ...
                        grid_of_types[index] = self.attributes.type_mag
                else:
    ...
    def set_rot_intertia_from_units(self):
        for x in self.attributes.realz_indices:
            sys.part[x].rinertia = np.ones(3) * 42.99375500352352
            sys.part[x].mass = 47.77083889280391

    def bond_relalz(self):
        self.attributes.bond = espressomd.interactions.FeneBond(
            k=self.attributes.fene_k, d_r_max=self.attributes.fene_r_max)
    ...
    def bond_rasp(self):
        ...
        sys.bonded_inter.add(self.attributes.bond)
        lebrow = [74+154*n for n in range(len(self.attributes.config))]
        leeyelash = [174+154*n for n in range(len(self.attributes.config))]
        print(lebrow)
        print(leeyelash)
        for x, y in zip(lebrow[:-1], leeyelash[:-1]):
            sys.part[x].add_bond((self.attributes.bond, y))
            print(str((x, y)))
        # for elem_1, elem_2 in zip(self.attributes.realz_indices[:-1],self.attributes.realz_indices[1:]):
        #     R=sys.part[elem_2].pos-sys.part[elem_1].pos
        #     all_the_relatives_1 = [x for x in sys.part[:] if x.vs_relative[0]==elem_1 and x.type==2]
        #     all_the_relatives_2 = [x for x in sys.part[:] if x.vs_relative[0]==elem_2 and x.type==2]
        #     lebrow=[np.linalg.norm((np.dot(x.pos-sys.part[elem_1].pos,R)/np.linalg.norm(R))*R) for x in all_the_relatives_1]
        #     leeyelash=[np.linalg.norm((np.dot(x.pos-sys.part[elem_2].pos,R)/np.linalg.norm(R))*R) for x in all_the_relatives_2]
        #     lebrow = [all_the_relatives_1[lebrow.index(x)].id for x in lebrow if np.allclose(np.linalg.norm(R)/2.,x,rtol=1.4e-1)]
        #     leeyelash = [all_the_relatives_2[leeyelash.index(x)].id for x in leeyelash if np.allclose(np.linalg.norm(R)/2.,x,rtol=1.4e-1)]
        #     print('lebrow'+str(lebrow))
        #     print('leeyelash'+str(leeyelash))
        #     minimisa=[np.linalg.norm(sys.part[leb_el].pos-sys.part[lee_el].pos) for (leb_el,lee_el) in product(lebrow,leeyelash)]
        #     elems=[(leb_el,lee_el) for (leb_el,lee_el) in product(lebrow,leeyelash)]
        #     # print(elems)
        #     # print(minimisa)
        #     print(elems[minimisa.index(min(minimisa))])
...
class Simulation():
    ...
    def set_sys(self, core):
        ...
        sys.virtual_sites = VirtualSitesRelative(
            have_velocity=True, have_quaternion=False)
        sys.seed = core.seed
    ...
    def init_lb_GPU(self, timestep):
        ...
        sys.part[:].v = (0, 0, 0)
        lbf = espressomd.lb.LBFluidGPU(kT=1, seed=core.seed, agrid=1, dens=1, visc=args.eta, tau=timestep)
    ...
    def filter_part_properties(self):
        ...
        prop_dict = dir(sys.part[0])
        stuffz = [x for x in prop_dict if (isinstance(getattr(sys.part[0], x), Iterable)
                                           or isinstance(getattr(sys.part[0], x), numbers.Number))]
        stuffz.remove('bonds')
        return stuffz
...
# %%
dir_path = os.getcwd()
path_to_custom_data = os.path.join(os.path.join('/home/lv70806/', dir_path[14:]), 'custom_dict_sphere.p.gz')
measurments_trj = open(
    os.path.join(os.path.join('/home/lv70806/', dir_path[14:]),
                 'trajectory_measured_sphere.vtf'),
    mode='w+t')

path_fluid_save = os.path.join(os.path.join('/home/lv70806/', dir_path[14:]), 'fluid_field.txt')
path_fluid_compressed = os.path.join(os.path.join('/home/lv70806/', dir_path[14:]), 'fluid_compressed.txt.gz')
path_fluid_sacrifice = os.path.join(os.path.join('/home/lv70806/', dir_path[14:]), 'you_cant_see_me.txt')
...
if args.h_field:
    sim_inst.set_H_ext(H=(args.h_field, 0, 0))
